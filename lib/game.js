...
class MirrorModel extends Croquet.Model {
    ...
    init() {
        ...
        this.subscribe('gameNetwork',"testalgo",this.testModelo);
    ...
    testModelo(){
        console.log('llego del view testalgo');
    }
    moveNetPlayer(data){
        console.log("update pos MODEL")
        this.publish('gameNetwork',"pouplateMovement",data);
    }
...
class ClientViews extends Croquet.View {
    ...
    testalgo(){
        this.publish('gameNetwork',"testalgo");
    }
    moveNetPhaserPlayer(data){
        console.log('me llega'+this.moveNetPhaserPlayer+'del model al view')
    }
